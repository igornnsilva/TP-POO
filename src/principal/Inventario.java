...
public class Inventario {
    ...
    public void desboloquearItemPadrao(int id){
        for(Item item: Itens){
            if(item.getIdItem() == id){
    ...
    public void desboloquearItem(int id, int nivelAtual){
        //Nivel nivel = this.getNiveis().get(nivelAtual-1);
        for(int i=0; i < nivelAtual; i++) {
            Nivel nivel = getNiveis().get(i);
            for (Item item : nivel.getItens()) {
                if (item.getIdItem() == id) {
                    item.setDesbloqueado(true);
                    System.out.println(item.getNome() + " desbloqueado.");
                    return;
                }
            }
        }
