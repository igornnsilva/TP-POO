...
public class PanelTrajetoria extends JPanel {
    ...
    public PanelTrajetoria() {
        ...
        // Define o layout como nulo para posicionamento manual
        this.setLayout(null);

        // Botão 'Fechar'
        JButton closeButton = new JButton("Fechar");
        closeButton.setBounds(900, 700, 100, 30); // Exemplo de posicionamento
        closeButton.addActionListener(e -> {
            // Fecha a janela que contém o painel
            SwingUtilities.getWindowAncestor(this).dispose();
        });
        this.add(closeButton); // Adiciona o botão
    ...
    private void initIcons() {
        ...
        for (int i = 0; i < icones.length; i++) {
            ImageIcon iconImage = new ImageIcon(getClass().getResource(iconPaths[i]));
            icones[i] = new JLabel(iconImage);
            icones[i].setBounds(startX + (i * step), startY, iconSize, iconSize); // Define a posição
            this.add(icones[i]); // Adiciona o ícone ao painel como JLabel
        }
    }
    ...
    protected void paintComponent(Graphics g) {
        ...
        for (int i = 0; i < icones.length - 1; i++) {
            g.drawLine(
                    startX + (i * step) + iconSize,
                    startY + iconSize / 2,
                    startX + ((i + 1) * step),
                    startY + iconSize / 2
            );
